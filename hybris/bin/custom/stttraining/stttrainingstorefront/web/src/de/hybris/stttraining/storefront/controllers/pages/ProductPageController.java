...
public class ProductPageController extends AbstractPageController
{
	...
	@Resource(name = "sstProductFacade")
	private SstProductFacade sstProductFacade;
			...
			throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		...
		// Check condition to query staged version or not
		final ProductData productData;
		if(stagedVersion)
			productData = sstProductFacade.getStagedProductForCodeAndOptions(productCode, extraOptions);
		else
		...
		if(stagedVersion)
			populateStagedProductDetailForDisplay(productCode, model, request, extraOptions);
		else
	...
	protected void populateStagedProductDetailForDisplay(final String productCode, final Model model, final HttpServletRequest request,
												   final List<ProductOption> extraOptions) throws CMSItemNotFoundException
	{
		final CatalogVersionModel catalogVersionModel = catalogVersionService.getCatalogVersion(
				StttrainingCoreConstants.CATALOG_VERSION_POWERTOOLS_PRODUCT_CATALOG_ID,
		...
		final ProductData productData = sstProductFacade.getStagedProductForCodeAndOptions(productCode, options);
